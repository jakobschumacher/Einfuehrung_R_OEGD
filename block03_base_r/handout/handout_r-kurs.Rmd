...
```{r learning progress, dpi=300, fig.align='center', fig.dim = c(4, 4), warning=FALSE}
# Daten für eine typische S-Kurve (logistische Funktion)
lernkurve <- tibble(
  tage = seq(1, 100, length.out = 100),
  fortschritt = (100 / (1 + exp(-0.08 * (tage - 50)))) # Logistische Funktion
)

ggplot(lernkurve, aes(x = tage, y = fortschritt)) +
  geom_line(color = "#1f77b4", size = 1.2) +
  geom_area(alpha = 0.2, fill = "#1f77b4") +
  labs(
    title = "Typische Lernkurve beim Lernen von R",
    x = "Zeit",
    y = "Fortschritt / Verständnis",
  ) +
  ...
  theme(
    ...
    axis.text.x = element_blank(),  # Vereinfacht
...
Sprachmodelle wie ChatGPT, Claude oder GitHub Copilot können beim Lernen von R und bei der praktischen Arbeit hilfreich sein. Sie können Ihnen bei häufigen Fragen helfen – zum Beispiel: „Wie lade ich eine CSV-Datei in R ein?" oder „Wie nutze ich `dplyr` zum Filtern von Daten?" – und sind oft schneller als Google. Sie können auch dabei helfen, Fehlermeldungen zu verstehen und zu beheben.

**Aber: KI hat klare Grenzen.** Diese Modelle funktionieren am besten bei etabliertem, gut dokumentiertem Wissen. Je häufiger etwas in Fachliteratur, Dokumentation und Online-Communities diskutiert wurde, desto besser kennt die KI es. Bei neueren Methoden, speziellen Packages oder gerade veröffentlichten Updates kann die KI veraltet, unpräzise oder einfach falsch sein – ohne dass es auf den ersten Blick offensichtlich ist.

**Deshalb:** Nutzen Sie KI als Lernhilfe und Schnellreferenz, nicht als einzige Quelle. Verifizieren Sie wichtige Antworten mit Ihrem eigenen Fachwissen, mit der offiziellen Dokumentation oder mit etablierten Ressourcen wie dem [Epidemiologist R Handbook](https://epirhandbook.com/en/) oder [R for Data Science](https://r4ds.hadley.nz/). Auch wenn die Antworten von KI immer gut und überzeugend klingen, bleiben Sie kritisch.
